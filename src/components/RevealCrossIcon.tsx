import React from 'react';

interface RevealCrossIconProps {
  isOpen?: boolean;
  className?: string;
  activeColor?: string;
  inactiveColor?: string;
}

export default function RevealCrossIcon({
  isOpen = false,
  className = "w-4 h-4",
  activeColor = "var(--brand-primary, #669ae1)",
  inactiveColor = "currentColor",
}: RevealCrossIconProps) {
  return (
    <svg
      viewBox="0 0 100 100"
      xmlns="http://www.w3.org/2000/svg"
      className={`${className} overflow-visible`}
    >
      {/* Center Block */}
      <rect
        className="rect-faq__block rect-faq__block--center"
        x="42"
        y="42"
        width="16"
        height="16"
        style={{
          fill: isOpen ? activeColor : inactiveColor,
          transition: "transform 0.4s cubic-bezier(0.16, 1, 0.3, 1), fill 0.3s ease",
        }}
      />
      {/* Top Block (making it a cross) */}
      <rect
        className="rect-faq__block rect-faq__block--top"
        x="42"
        y="22"
        width="16"
        height="16"
        style={{
          transform: isOpen ? "translateY(20px)" : undefined,
          fill: isOpen ? activeColor : inactiveColor,
          transition: "transform 0.4s cubic-bezier(0.16, 1, 0.3, 1), fill 0.3s ease",
          transformOrigin: "center",
        }}
      />
      {/* Bottom Block */}
      <rect
        className="rect-faq__block rect-faq__block--bottom"
        x="42"
        y="62"
        width="16"
        height="16"
        style={{
          transform: isOpen ? "translateY(-20px)" : undefined,
          fill: isOpen ? activeColor : inactiveColor,
          transition: "transform 0.4s cubic-bezier(0.16, 1, 0.3, 1), fill 0.3s ease",
          transformOrigin: "center",
        }}
      />
      {/* Left Block */}
      <rect
        className="rect-faq__block rect-faq__block--left"
        x="22"
        y="42"
        width="16"
        height="16"
        style={{
          transform: isOpen ? "translateX(20px)" : undefined,
          fill: isOpen ? activeColor : inactiveColor,
          transition: "transform 0.4s cubic-bezier(0.16, 1, 0.3, 1), fill 0.3s ease",
          transformOrigin: "center",
        }}
      />
      {/* Right Block */}
      <rect
        className="rect-faq__block rect-faq__block--right"
        x="62"
        y="42"
        width="16"
        height="16"
        style={{
          transform: isOpen ? "translateX(-20px)" : undefined,
          fill: isOpen ? activeColor : inactiveColor,
          transition: "transform 0.4s cubic-bezier(0.16, 1, 0.3, 1), fill 0.3s ease",
          transformOrigin: "center",
        }}
      />
    </svg>
  );
}
